...
FACES_MAPPING = {0: "red", 1: "blue", 2: "black", 3: "white"}
FORKED_BLOCKCHAIN_ENVIROMENTS = []
...
def get_face(face_num):
    return FACES_MAPPING[face_num]


#########################################################################################
def get_account():
    if network.show_active() in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        # account = accounts.load("cweetbeatz6")
        account = accounts[0]
        return account
    else:
...
def deploy_mocks():
    ...
    print(f"Link Address: {link_token.address}")
